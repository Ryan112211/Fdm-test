<template>
	<div
		class="w-full max-w-sm p-4 m-4 h-[700px] bg-white border border-gray-200 rounded-lg shadow sm:p-8 dark:bg-gray-800 dark:border-gray-700"
	>
		...
		<input
			v-model="inputValue"
			type="text"
			class="mt-4 w-full px-3 py-2 text-gray-700 border rounded-lg focus:outline-none"
		...
		<div class="mt-4 text-sm text-white dark:text-white-400" v-if="error">{{ error.message }}</div>
		<!-- <div class="mt-4 text-sm text-white dark:text-white-400" v-if="data">{{ data.message }}</div> -->
		<div class="mt-4 text-sm text-white dark:text-white-400" v-if="data.length">{{ data[data.length - 1].name }}</div>
